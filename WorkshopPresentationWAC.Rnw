...
\begin{block}{Three Wind-Measuring Systems:}
\begin{enumerate}
\item The standard system:
\begin{enumerate}
\item radome + pitot tube and static ports for ``relative wind''
\begin{itemize}
\item New option: substitute a pitot-static tube for the pitot tube
\end{itemize}
...
858 probe.
\begin{itemize}
\item Requires significant corrections for flow distortion
\end{itemize}
\end{enumerate}
\end{block}
\end{frame}
...
\begin{summaryblock}{Standard System (without new pitot-static sensor):}
\begin{itemize}
\item Measurement uncertainty: documented in NCAR Tech Note, 2016

\href{https://opensky.ucar.edu/islandora/object/technotes\%3A540}{https://opensky.ucar.edu/islandora/object/technotes\%3A540}

claimed uncertainty limits are <0.4~m/s (horizontal components) and
<0.2~m/s (vertical)
\item with Kalman filter corrections: about 0.1~m/s

\href{https://opensky.ucar.edu/islandora/object/technotes\%3A559}{https://opensky.ucar.edu/islandora/object/technotes\%3A559}
...
\begin{block}{Laser Air Motion Sensor:}
\begin{itemize}
\item Primary use has been to ``calibrate'' the standard system:
\begin{itemize}
\item Corrected the airspeed measurement for effects of the ``static defect.''
\item Verified parameterized representations of angle of attack and sideslip.
\end{itemize}
\item Other potential uses. The next talks provide more information.
\end{itemize}
\end{block}
\begin{summaryblock}{Under-Wing All Weather Pod:}
\begin{itemize}
\item With large pods installed, flow-distortion effects cannot be corrected
adequately.
\item With only small pods, usable results are produced, and the small size
of the sensor and the short line lengths offer some advantages for
high-frequency measurements.
\end{itemize}
\end{summaryblock}
\end{frame}
%
\begin{frame}[fragile]{VARIANCE SPECTRA, HIGH-TURBULENCE REGION, with Pitot-Static Sensor}
...
DSRT8$AK <- cff * DSRT8$QRF * 1.1 + cfs[1] + cfs[2] * DSRT8$QRS + cfs[3] * DSRT8$QCFS
...
DSRT8$SSRD <- -0.008 + 22.302 * (DSRT8$BDIFR / DSRT8$QCF) * 0.95
...
@vspec, include=FALSE, echo=FALSE>>=
...
VSpec <- function (.data, .Variable, VLabel=NA, col=NA, type='spectrum', method=NA, xlim=c(0.001, 15), ylim=c(0.0001,1),
  ...
  rg <- freq > xlim[1] & freq < xlim[2]
  DF <- data.frame(freq[rg], fpf[rg])
  if (is.na(ADD[1])) {
...
<<plotHTurb, include=TRUE, echo=FALSE, fig.height=5>>=

D$UXTCA <- D$UXTC * sqrt(4/3)
g <- VSpec(D, 'WIC', method='MEM', xlim=c(0.01,10), ylim=c(0.001,1))
